...
class DirectoryMonitorInputEvent(FileMonitorInputEvent):
    """
    ...
    def __init__(self, monitored_directory: AsyncPath, changed_path: AsyncPath) -> None:
        super().__init__(file=changed_path)
        self.dir = monitored_directory
...
class DirectoryMovedIntoOrFromWatchedDirInputEvent(DirectoryMonitorInputEvent):
    ...
    def __init__(
        self, monitored_directory: AsyncPath, changed_path: AsyncPath, source_path: AsyncPath
    ) -> None:
        super().__init__(monitored_directory=monitored_directory, changed_path=changed_path)
        self.dir_moved_from = source_path
...
class FileCreatedInMonitoredDirectoryInputEvent(DirectoryMonitorInputEvent):
    """
...
class FileUpdatedInMonitoredDirectoryInputEvent(DirectoryMonitorInputEvent):
    """
    Event called when a dir comes into existence at the monitored location - the input location.
    Note - the input cannot distinguish between a dir being created or moved to the input location.
    """
...
class FileDeletedFromMonitoredDirectoryInputEvent(DirectoryMonitorInputEvent):
    """
...
class FileMovedIntoOrFromWatchedDirInputEvent(DirectoryMonitorInputEvent):
    """???"""
...
class DirectoryMonitorInput(Input):
        ...
        if await target_async_path.is_file():  # Not sure this case should ever be reached
            ...
            await self.send(
                FileCreatedInMonitoredDirectoryInputEvent(
                    ...
                    changed_path=target_async_path,
                )
            )
    ...
    async def send(self, event: FileMonitorInputEvent) -> None:
    ...
    async def monitor_dir_watcher(self) -> bool:
        ...
        if dir_deleted:
            ...
            await self.send(
                FileDeletedFromMonitoredDirectoryInputEvent(
                    monitored_directory=AsyncPath(self._input_path),
                    changed_path=AsyncPath(self._input_path),
                )
            )
            return False
        return True
    ...
    async def _process_dir_event(self, event: FileSystemEvent) -> None:
        ...
        if isinstance(event, watchdog.events.DirCreatedEvent):
            await self.send(
                DirectoryCreatedInWatchedDirInputEvent(
                    monitored_directory=AsyncPath(self._input_path),
                    changed_path=AsyncPath(event.src_path),
                )
            )
            ...
            await self.send(
                DirectoryMovedIntoOrFromWatchedDirInputEvent(
                    monitored_directory=AsyncPath(self._input_path),
                    changed_path=AsyncPath(event.dest_path),
                    source_path=AsyncPath(event.src_path),
                )
            )
    ...
    async def _process_file_modified_event(
        ...
        if await file_async_path.exists():
            ...
            await self.send(
                DirectoryInMonitoredDirectoryWasUpdatedInputEvent(
                    monitored_directory=self._input_path,
                    changed_path=AsyncPath(dir_path),
                )
            )
        ...
        if event.src_path in self._python_registers_deleted_cache:
            ...
            if event.src_path in self._dir_cache:
                ...
                await self.send(
                    DirectoryInMonitoredDirectoryWasUpdatedInputEvent(
                        monitored_directory=AsyncPath(self._input_path),
                        changed_path=AsyncPath(event.src_path).parent,
                    )
                )
...
class LinuxFileSystemObserver(BaseObserver):
    ...
    async def _process_file_modified_event(
        ...
        await self.send(
            FileUpdatedInMonitoredDirectoryInputEvent(
                monitored_directory=self._input_path, changed_path=file_async_path
            )
        )
