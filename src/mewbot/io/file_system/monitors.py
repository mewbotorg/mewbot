...
from mewbot.io.file_system.events import (
    ...
    DirectoryInMonitoredDirectoryWasUpdatedInputEvent, FileCreatedInMonitoredDirectoryInputEvent,
)
...
class BaseObserver:
    ...
    async def monitor_dir_watcher(self) -> bool:
        ...
        if dir_deleted:
            ...
            await self.send(
                FileDeletedFromMonitoredDirectoryInputEvent(
                    monitored_directory=aiopath.AsyncPath(self._input_path),
                    deleted_path=aiopath.AsyncPath(self._input_path),
                )
            )
            return False
        return True
    ...
    #
    #         await self.send(FileCreatedInWatchedDirInputEvent(file=target_async_path, base_event=None))
    #
    #     else:
    #         self._logger.warning(
    ...
    async def _process_dir_event(self, event: FileSystemEvent) -> None:
        ...
        if isinstance(event, watchdog.events.DirCreatedEvent):
            await self.send(
                DirectoryCreatedInWatchedDirInputEvent(
                    dir=aiopath.AsyncPath(event.src_path),
                    base_event=event,
                )
            )
            ...
            await self.send(
                DirectoryMovedIntoOrFromWatchedDirInputEvent(
                    dir_moved_from=aiopath.AsyncPath(event.src_path),
                    dir=aiopath.AsyncPath(event.dest_path),
                    base_event=event,
                )
            )
...
class WindowsFileSystemObserver(BaseObserver):
    ...
    async def _process_file_creation_event(
        ...
        await self.send(
            FileCreatedInMonitoredDirectoryInputEvent(
                file=aiopath.AsyncPath(event.src_path),
                base_event=event,
            )
        )

    async def _dispatch_file_created(self, event: watchdog.events.FileSystemEvent, file_async_path: aiopath.AsyncPath) -> None:
    ...
    async def _process_file_modified_event(
        ...
        if await file_async_path.exists():
            ...
            await self.send(
                DirectoryInMonitoredDirectoryWasUpdatedInputEvent(
                    dir=aiopath.AsyncPath(dir_path),
                    base_event=None,
                )
            )
    ...
    async def _process_file_delete_event(
        ...
        if event.src_path in self._python_registers_deleted_cache:
            ...
            if event.src_path in self._dir_cache:
                ...
                await self.send(
                    DirectoryInMonitoredDirectoryWasUpdatedInputEvent(
                        dir=aiopath.AsyncPath(self._input_path),
                        base_event=None,
                    )
                )
...
class LinuxFileSystemObserver(BaseObserver):
    ...
    async def _process_file_move_event(self, event: watchdog.events.FileSystemMovedEvent) -> None:
        # TODO: Test moving into, internal, and out of the watched directory
        await self.send(
            FileMovedIntoOrFromWatchedDirInputEvent(
                file=aiopath.AsyncPath(event.dest_path),
                file_src=event.src_path,
                base_event=event,
            )
        )

    async def _process_file_delete_event(self, event: watchdog.events.FileDeletedEvent) -> None:
        await self.send(
            FileDeletedFromMonitoredDirectoryInputEvent(
                monitored_directory=aiopath.AsyncPath(self._input_path),
                deleted_path=aiopath.AsyncPath(event.src_path),
            )
        )
